...
from utils.test_util import test_with_ocr, load_model

def main(cfg):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    model = load_model(f"{cfg['saved_dir']}/weights/best.pth", cfg['model_name'], 17, device)
    test_with_ocr(model, cfg["test_img_path"], cfg['test_csv_path'], f"{cfg['saved_dir']}/preds", cfg['img_h'], cfg['img_w'], device)
