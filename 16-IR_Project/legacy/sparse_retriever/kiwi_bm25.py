...
class KiwiBM25Retriever(BaseRetriever):
    ...
    def search_with_score(self, query: str, top_k=None):
        ...
        for i, doc in enumerate(self.docs):
            # 기존 문서의 메타데이터를 가져오고, score와 함께 새로운 메타데이터로 추가
            metadata_with_score = {**doc.metadata, "score": normalized_score[i]}
            document = Document(
                page_content=doc.page_content, metadata=metadata_with_score
            )
